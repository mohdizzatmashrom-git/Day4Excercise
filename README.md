...
### Exercise 06 - Search Filter UI


How is JavaScript filter used in a search feature?

The JavaScript `filter` method is used to search through an array and return only the elements that match a specific condition. In this search feature, `filter` goes through the `students` array one by one and checks whether each student's name (converted to lowercase) includes the search term (also converted to lowercase). If the condition returns `true`, that student is kept in the new filtered array. If it returns `false`, the student is excluded. The filtered array is then passed to `renderStudents()` to display only the matching results on the page.
